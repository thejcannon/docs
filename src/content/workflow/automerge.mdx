   ...
   pull_request_rules:
     - name: Automatic merge when CI passes and reviews approve
       conditions:
         ...
         - status-success=CI # replace "CI" with the name of your CI status check
       actions:
         merge:
           method: merge
   ```
...
3. **Customize Your Merge Action**: The `merge` action in the rule can be
   ...
   the [merge action](/actions/merge/) documentation
...
action](/actions/queue) to put pull requests in the merge queue rather than
merging directly.
...
(`#approved-reviews-by>=2` and `status-success=CI`), it is added to the merge
