...
- `partition_rules` configures [monorepository support in the merge
  queue](/merge-queue/partitions);
...
- `extends` allows to [extend the
  configuration](file-format#extending-configuration-files) file by importing
...
your `.mergify.yml` file. Specify the repository name from which you want
to include the configuration. The configuration from the specified repository
will be loaded and applied before the one in the current repository.
