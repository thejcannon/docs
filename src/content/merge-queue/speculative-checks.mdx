...
merge serially can be time-consuming, especially when each merge requires
passing through continuous integration (CI) pipelines. This is where
**Speculative Checks** come in.

Speculative checks, as part of Mergify's queuing feature, allow for the
parallel testing of multiple PRs to expedite the merging process. They leverage
the concept of a **merge train** - a set of PRs tested together to ensure
compatibility before merging.

Essentially, speculative checks help accelerate the PR merging process by
preemptively checking for potential merge conflicts or test failures, saving
valuable development time. This introduction will guide you through
understanding speculative checks, how they operate, and how to configure them
effectively.
...
If an embarked PR no longer matches the `routing_conditions`, it is removed
...
commit or a change in labels) or changes to the `routing_conditions`. In either
...
When combined with [priority rules](priority-rules), speculative checks become
even more powerful. You can define priority for PRs so that urgent ones are
merged faster, while the rest are tested speculatively in the background.
...
{/* FIXME: link to settings */}

Branches created by Mergify are prefixed with `mergify/merge-queue/` by default
and can be change with the `queue_branch_prefix` settings.
