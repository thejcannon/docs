...
3. **Meeting routing conditions:** Before a pull request can be added to the
   merge queue, it must meet certain conditions, specified under
   `routing_conditions` in your `queue_rules`. These conditions might include
...
4. **Entering the queue:** Once a pull request meets the specified routing
   conditions, it is added to the end of the merge queue. However, priority
   rules can be used to alter its position in the queue.

{/* FIXME: add link to batch */}
...
base branch. You can test multiple pull requests at once by using batches or in
advance by using [speculative checks](speculative-checks).
...
In case any of the `routing_conditions` stop being true during the update,
...
1. **Non-compliance with `routing_conditions`:** If a pull request no longer
   meets the `routing_conditions` specified in the Mergify configuration file,
   it will automatically be removed from the queue. For instance, if one of the
   `routing_conditions` is the presence of a specific label, and that label
...
back in the merge queue according to the queue's `routing_conditions`.
...
in a queue matching its `routing_conditions`.
