...
met before a PR is added to the queue (`routing_conditions`) and when it can be
...
If a PR stops matching its `routing_condition`, it will be removed from the
queue. That means there is no need to repeat the `routing_conditions` in the
...
while those that have met all their `routing_conditions` are displayed in the
